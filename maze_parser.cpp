...
// Arc structure: Used to keep track of the arcs for the adjacency list.
struct Arc {
    ...
    // Full Initialization
    Arc(struct Arc* successor, uint32_t node_index, uint32_t displacement) {
        next = successor;
        end_node = node_index;
        weight = displacement;
    }
...
// Modification: Changed AdjacenyEntry to Node to begin to replicate
//               homework design.
struct Node {
    ...
    Node(uint32_t index, uint32_t row, uint32_t col, uint32_t value) {
        first = NULL;
        priority = UINT32_MAX;
        node_index = index;
        maze_row = row;
...
typedef std::map<uint32_t, Node> NodeIndexToCellMap;
typedef std::vector<std::vector<MazeCell*> > MazeMatrix;
typedef std::map<uint32_t, std::vector<Node*> > AdjacencyList;
typedef std::vector<std::vector<bool> > AdjacencyMatrix;
typedef std::vector<std::vector<Node> > AdjacencyTravelMatrix;
...
public:
    MazeTraveler(MazeMatrix matrix, const AdjacencyList& adj_list) {
        ...
        for (AdjacencyList::const_iterator u = adj_list.begin();
                u != adj_list.end(); u++) {
            for (std::vector<Node*>::const_iterator v =
    ...
    void TravelToNode(uint32_t node_index, const char* stroke_color) {
        ...
        Node adj_entry = _adj_matrix[origin_node_index][node_index];
...
int main(int argc, char** argv) {
        ...
        for (int col = 0; col < col_count; col++) {
            ...
            if (src_cell == dest_cell) {
                ...
                adjacency_list[src_cell->GetNodeIndex()].push_back(
                        new Node(dest_cell->GetNodeIndex(), row, col,
                                num_hops));
                adjacency_list[dest_cell->GetNodeIndex()].push_back(
                        new Node(src_cell->GetNodeIndex(), row,
                                src_col, num_hops));
                num_hops = 0;
                if (dest_cell->CanGoRight()) {
                    src_cell = dest_cell;
            ...
            if (src_cell == dest_cell) {
                ...
                adjacency_list[src_cell->GetNodeIndex()].push_back(
                        new Node(dest_cell->GetNodeIndex(), row, col,
                                num_hops));
                adjacency_list[dest_cell->GetNodeIndex()].push_back(
                        new Node(src_cell->GetNodeIndex(), src_row,
                                col, num_hops));
                num_hops = 0;
                if (dest_cell->CanGoDown()) {
                    src_cell = dest_cell;
    ...
    for (AdjacencyList::iterator u = adjacency_list.begin();
        ...
        for (std::vector<Node*>::iterator v = u->second.begin();
                v != u->second.end(); v++) {
            fprintf(stdout, "--> %u(%u)", (*v)->node_index, (*v)->start_dis);
        }
        fprintf(stdout, "\n");
    }
